# Load data into Azure DocumentDB
...
This data will be used in subsequent labs to explain further the capabilities of Azure DocumentDB with respect to AI.
...
## Lab - Load data into Azure DocumentDB collections
...
Please visit the lab repository to complete [this lab](/vcore/Labs/lab_2_load_data.ipynb).
...
As an example, the following code snippet inserts product data using the `bulk_write` method allowing for upsert functionality using the `UpdateOne` method:
...
bulk_operations = [ UpdateOne({"_id": prod.id}, {"$set": prod.model_dump(by_alias=True)}, upsert=True) for prod in product_data.items]
db.products.bulk_write(bulk_operations)
```

The lab continues with bulk loading customer and sales data, this time with the `insert_many` method.
The insert_many method is used to insert multiple documents into a collection, it differs from the bulk_write method in that it only supports insert operations.
The following code snippet demonstrates the use of the simplified `insert_many` method to insert customer data:

```python
result = await db.customers.insertMany(customerData)
```
